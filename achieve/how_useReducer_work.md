...
`React` 使用环状链表保存更新队列 `queue={ pending: null }`，
queue是一个环创链表
其中 `pending` 永远指向最后一个更新。比如多次调用 `setState` 时：
...
在 `hook` 对象中，`hook.memoizedState` 属性用于保存当前状态，比如 `hook1.memoizedState` 对应的就是 `count`。
`hook1.next` 指向 `hook2`。`hook1.queue`保存的是调用 `setCount` 后的更新队列。
...
通过容器节点 `root` 查找对应的 `fiber` 节点 输入console.dir(root)

另一种方法是在源码中打断点，这个需要了解源码。
在`react-dom.development.js`中搜索`renderWithHooks`方法，

在 `var children = Component(props, secondArg)` 处打一个断点，然后在它下面一行再打一个断点，等 `Component(props, secondArg)` 函数执行完成，
则 `hook` 链表构造完成，此时可以在控制台打印`console.log(workInProgress)`即可看到当前 `fiber` 节点的信息
...
`ReactCurrentDispatcher` 变量设置为 `HooksDispatcherOnMount`，如果是更新阶段，则设置为 `HooksDispatcherOnUpdate`。
这样当我们调用 `useReducer(reducer, 0)`时，实际上调用的是 `HooksDispatcherOnMount.useReducer` 或者 `HooksDispatcherOnUpdate.useReducer`
...
- 重置 `fiber` 的 `hook` 链表为 `null`。`workInProgress.memoizedState = null`。
	- 更新阶段一样会重置 `hook` 链表并重新生成
- 设置 ReactCurrentDispatcher。
	- 通过 current决定是调用 `mountReducer` 还是 `updateReducer`
	- 如果是初次挂载阶段，则设置为 `HooksDispatcherOnMount`，
	- 更新阶段则设置为 `HooksDispatcherOnUpdate`。以此
...
beginWork(current, worklnProgress, renderLanes)
- 根据worklnProgress.tag判断这是更新阶段，因此调用updateFunctionComponent方法
updateFunctionComponent
renderWithHooks
- Component(props,secondArg)实际上就是我们的函数组件Counter
- Component内部调用useReducer,实际上调用的就是HooksDispatcherOnUpdate.useReducer
updateReducer
updateWorklnProgressHook

### 第四节 hook 主流程源码实现

ReactFiberBeginWork.js 
主要是根据不同的 WIP.tag判断是更新还是初次挂载阶段
...
---
updateReducer和updateWorkInProgressHook
```js
