...
当使用`webpack-dev-server`时，考虑代码改动到效果呈现的过程。 代码变动后重新打包，浏览器会刷新重新请求所有资源

而使用了热替换后，流程发生了变化。浏览器变成了仅请求改动的资源
...
当开启了热更新后，`webpack-dev-server`会向打包结果中注入`module.hot`属性，因此就可以最终代码就会执行accept函数
