...
**1、为什么 Fiber 架构更快？**
在上面这段代码里，
- 我们还可以看出来为什么 Fiber 架构比以前的递归 DOM 计算要快：**`flags`** 或 **`subtreeFlags`** 是 16 进制的标识，在这里进行按位或 (`|`) 运算后，可以记录当前节点本身和子树的副作用类型，通过这个运算结果可以减少节点的遍历，举一个简单的例子说明：
...
Part 2 Fiber 树的遍历方式
-------------------
前面说过：「React 构建出新 Virtual DOM 树，通过 Diffing 算法和老树对比」。
...
next 先从 beginWork 获取，如果没有，就从 completeUnitOfWork 获取。
...
![](_resources/v2-eabcf03d0c43acfedb92c3933575c8f9_r.jpg.png)
...
![](_resources/v2-ab587b626f5a0af15dac6fb9a825feeb_r.jpg.png)

如果构建到一半被打断，current 树还在。如果构建并提交完成，直接把 current 树丢掉，让 workInProgress Fiber 树成为新的 current 树。

![](_resources/v2-48ee0248ff53e2e3ce1280f5b9de57f1_r.jpg.png)

所谓 Diffing 也是在这两棵树之间，如果构建过程中确认新节点对旧节点的复用关系，新旧节点间也会通过 alternate 指针相连。

![](_resources/v2-422ba239fe44fa9c8745039cc3b6d6b9_r.jpg.png)
