...
> 最近有人在脸书前端交流社群发了一则贴文，内容是他看到了一个问题：请问登入 api 传账号、密码 json 明码会有问题吗?，想知道大家对这个问题的看法。

最近有人在脸书前端交流社群发了一则贴文，内容是他看到了一个问题：[请问登入 api 传账号、密码 json 明码会有问题吗？](https://www.facebook.com/groups/f2e.tw/posts/5689037364466915)，想知道大家对这个问题的看法。

而底下的回答大部份都是觉得「有用 HTTPS 就好了，没必要额外再实作一层加密，没有什么太大的意义」

而底下的回答大部份都是觉得「有用 HTTPS 就好了，没必要额外再实作一层加密，没有什么太大的意义」

老实说我以前也是这样认为的，而且过去在社群中就有出现过类似的讨论。 我那时候想说都已经有 HTTPS 了，而 HTTPS 本身的目的就是为了保障传输的安全，为什么要自己再做加密？
...
在进入正题之前，我想把问题定义得更明确一点，免得出现一堆张飞打岳飞，拿两个完全不同的情境来比较的状况。 在原文底下，就有不少留言在讨论的是不同的问题，把问题先定义清楚满重要的。
...
这边需要注意的是「两个状况都是 HTTPS」，所以如果你想讲的是「没必要自己发明新技术」或是「自己发明新的加密方式不会比较安全」等等，在这个前提下全都不适用。

因为传输层还是靠 HTTPS 在传输，所以在这个阶段并没有自己发明新的方式，我只是在应用层自己把传输的数据额外加密一层而已。

再来，先不考虑成本这件事，纯粹只从技术的角度去看可能有哪些优缺点（成本相关的最后再来讨论）。

然后这篇也不讨论「保护帐号安全」这件事情，纯粹在讨论「保护密码」。 举例来说，有人会讲说：「密码加密有什么用？ 多此一举而已，先做 2FA 比较实在」，这种就是超出范围的讨论。 2FA 或 MFA 属于「保护帐号安全」，就算密码被偷走，攻击者也会被其他验证所挡住，因此没有办法登入进去帐号。

这与「保护密码」是完全不同的两件事情，因为实作 MFA 对于保护密码没什么帮助。 很简单就可以假设出一种状况是虽然攻击者无法登入账号，但密码明文还是被偷走了。

用户可能会在多个服务使用同一组密码，当用户在你网站的密码被偷走，影响到的不只是他在这边的帐号，还有可能影响到其他服务，这就是为什么我们在讨论「保护密码」。

最后呢，我这边的情境是「加密密码」而非 hash，这是因为我觉得 hash 的状况比较复杂，我自己想先用加密来举例，而且这个加密是「非对称式加密」。

也就是我们可以想象已经有一把 publickey 存在于 client 端（当然，每个人都可以拿得到），在送出 request 以前，会先使用 JavaScript 把密码用 public key 加密以后再送出，而 server 使用 private key 解密，拿到密码以后 hash 过再存入数据库中。

综合以上，这篇想处理的问题是：「已经使用了 HTTPS，在打登入 API 以前先把密码加密过或是不做任何处理，这两者的差别在哪？」

而我们可以把回答分成两个部分：

1.  假设 HTTPS 被破解了，是不安全的，差在哪？
2.  假设 HTTPS 是安全的，差在哪？

假设 HTTPS 不安全，差在哪？
-----------------

首先，可以先想一下怎样的状况会造成 HTTPS 不安全，攻击者掌握了系统的哪些部分？
...
那如果是「攻击者成功执行了中间人攻击（Man-In-The-Middle）」呢？ 你的计算机没事，只是数据包在传输的过程中被中间人攻击。

在这样的前提底下，没加密的情形就能直接被获取密码，而有加密的情形攻击者只能获取到加密过的密文而非明文。 但需要注意的是既然是叫中间人攻击，那攻击者除了监听你的 request 以外，也能发送伪造的 response 给你，把前端用来加密密码的部分换掉。
...
此时你可能会想问的问题是：「那在什么状况下攻击者能够取得 HTTPS 的明文？ 真的有可能吗？」

这边有一份美国卫生及公共服务部 HHS 所做的演示文稿：[SSL/TLS Vulnerabilities](https://www.hhs.gov/sites/default/files/securing-ssl-tls-in-healthcare-tlpwhite.pdf)，里面有记录一些 SSL/TLS 曾经有过的漏洞，所以取得 HTTPS 的明文确实是有可能的。

不过光是知道「有可能」是不够的，应该问的是「概率高吗？」 ，在讨论风险的时候，通常也会根据风险的高低以及严重程度来决定该怎么处理这个风险。

答案是「概率很低」，演示文稿里的漏洞最近的已经是 2017 年的事情了，而且是使用一些旧的、有问题的加密演算法，除此之外还需要符合不少其他条件，才能执行攻击，所以我认为概率确实是很低的。

以 2016 年发表的 [DROWN（Decrypting RSA with Obsolete and Weakened eNcryption）](https://drownattack.com/) 来说，Server 要支持 SSLv2，而攻击者要能抓到加密过的 TLS connection，符合这些条件以后，进行一大堆运算后就可以解开 900 个 connection 中的 1 个， 而运算成本在当时是 440 美金，约 13k 台币。
...
或许是因为觉得 HTTPS 被攻破的可能性太低，低到可以忽略不计（我相信这是大多数留言的人觉得不需要做的理由，我也同意这点），也或许是成本太高，会增加程序码复杂度；若是采用加密的方案，也会消耗更多运算的资源在加解密上面，这也是成本。
